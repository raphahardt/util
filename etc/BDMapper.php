...
class BDMapper extends Mapper implements DatabaseItfMapper {
  
  const DEFAULT_ID_NAME = 'id';
  const DEFAULT_DELETE_NAME = 'excluido';
  const DEFAULT_DELETE_DATE_NAME = 'excluido_em';
  const MAX_LIMIT_COLLECTION = 5000001; // sempre deixe um número redondo + 1
  ...
  public function init($config = null) {
    if (!isset($this->bd))
      $this->bd = BD::getInstance($config);
    
    if (!isset($this->entity))
      throw new CoreException('Obrigatorio definir uma tabela');
    
    if (!isset($this->fields)) {
      if ($this->entity instanceof SQLProcedure)
        $this->setFields($this->entity->Params);
      else
        $this->setFields($this->entity->Fields);
    }
  ...
  public function setFilter($constraints) {
  ...
  public function select() {
    ...
    if ($this->entity instanceof SQLProcedure) {
      // procedure
      $sup_instruction = new SQLICall($this->entity, $this->fields);
      ...
      $instruction = new SQLISelect(array_merge($this->fields, array($ftotal, $frn)), $this->entity, $where, $this->order);
    ...
    if ($success = $success && $bd->prepare($sql)) {
      foreach ($bind_v as $k => $value) {
        if (strpos($k, ':cursor_') !== false) {
          // se for cursor, jogar na var de cursor
          $bd->bind_param_cursor($k);
        } else {
          // binda o valor
  ...
  public function insertOne() {
    ...
    foreach ($all_data as $block) {
      ...
      $instr_returning = new SQLIReturning($this->fields[key($this->pointer)], $this->fields[key($this->pointer)]);
      $instr = new SQLIInsert($this->entity, $this->_getUpdatedValues(), $instr_returning);
      ...
      if ($success = $success && $bd->prepare($sql)) {
        foreach ($bind_v as $k => $value) {
          if (substr($k, 0, 6) === ':into_') {
            ...
            $k_field = substr($k, 6);
            $bd->bind_param($k, $returning_into[$k_field], 4000); // para valores out, deve-se garantir o espaço
          } else {
            // binda o valor
            $bd->bind_param($k, $bind_v[$k]);
          }
        ...
        if ($success = $success && $bd->execute()) {
          ...
          if ($affected > 0) {
            ...
            if ($need_id) {
              ...
              $this->data[key($this->pointer)] = $returning_into[key($this->pointer)];
  ...
  public function update() {
    ...
    $this->_to_dump($this->getPointer(), $this->getPointerValue());
