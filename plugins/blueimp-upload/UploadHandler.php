...
class UploadHandler {
  ...
  protected function get_file_size($file_path, $clear_stat_cache = false) {
    if ($clear_stat_cache) {
      @clearstatcache(true, $file_path); // fix bug
    }
  ...
  protected function handle_file_upload($uploaded_file, $name, $size, $type, 
    ...
    if ($this->validate($uploaded_file, $file, $error, $index)) {
      ...
      if ($file_size === $file->size) {
        $file->upload_to_db = $this->add_db($file, $index);
        $file->url = $this->get_download_url($file->name);
      ...
      //$file->upload_to_db = $this->add_db($file); // TODO: verificar se essa linha não é dentro do if $filesize===$file.size
      $this->set_additional_file_properties($file);
    }
    return $file;
  }
  ...
  public function delete($print_response = true) {
    ...
    foreach ($file_names as $file_name) {
      $file_path = $this->get_upload_path($file_name);
      // remove o registro na tabela e também apaga o arquivo
      $success = $this->remove_db($file_name) &&
  ...
  protected function add_db($file, $index) {
    // fazer aqui as regras de banco para adicionar o registro da tabela
    if (!defined('DJCK'))
      return false;
    return true;
  }
  ...
  protected function remove_db($filename) {
    // fazer aqui as regras de banco para apagar o registro da tabela
    if (!defined('DJCK'))
      return false;
    return true;
  }
