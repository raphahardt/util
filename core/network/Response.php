...
class Response {
  ...
  protected function _setCookies() {
    foreach ($this->_cookies as $name => $c) {
      setcookie(
              $name, $c['value'], $c['expire'], $c['path'], $c['domain'], $c['secure'], $c['httpOnly']
      );
    }
  }
  ...
  public function header($header = null, $value = null) {
    ...
    if (is_array($header)) {
      foreach ($header as $h => $v) {
        ...
        // predefine algumas coisas internas
        // TODO: também setar para Content-Length, Content-...
        if (strtolower($h) === 'content-type') {
          list($_type, $_charset) = explode(';',$v, 2);
          $this->type($_type);
        }
    ...
    if (strtolower($header) === 'content-type') {
      list($_type, $_charset) = explode(';',$value, 2);
      $this->type($_type);
    }
    return $this->_headers;
  }
