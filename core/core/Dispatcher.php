...
class Dispatcher {
  ...
  protected function _countRouteIteration() {
    static $iterations = 0;
    ++$iterations;
    
    if ($iterations > 10) {
      throw new DispatcherException('A rota entrou em loop');
    }
  }
  ...
  protected function getOutputBufferContents() {
    return ob_get_clean();
  }
  
  protected function endOutputBuffer($success) {
    if ($success) {
      ob_end_flush();
    } else {
      ob_end_clean();
  ...
  public function route($target, $params=array(), $response_code = null) {
    $this->_countRouteIteration();
    
    // pega a ação e o controller a ser usado
    if (is_array($target))
      list($class_name, $action) = each($target);
    ...
    try {
      ...
      foreach ($rflc_parameters as $rflc_param) {
        ...
        switch ($param_name) {
          ...
          case 'token':
            ...
            $params_to_pass[] = filter_var($class->Request->header('X-Djck-Token'), 
                    FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH | FILTER_FLAG_STRIP_LOW);
            break;
          case 'statusCode':
            $params_to_pass[] = $class->Response->statusCode();
            break;
          default:
      ...
      try {
        ...
        $class->__call($action, $params_to_pass);
      }
      catch (\Exception $e) {
        $this->route('error', null, 500);
        return;
      }

      // pega o conteudo do buffer
      //$contents = $this->getOutputBufferContents();
      //$this->endOutputBuffer(true);
      
      // manda os headers definidos automaticamente
      // FIX: não mando mais o conteudo no body(), pois ob_get_contents() não chama
      // o callback definido no ob_start(), portanto eu mando primeiro os headers
      // normalmente e depois echo-o o conteudo com o flush mesmo
